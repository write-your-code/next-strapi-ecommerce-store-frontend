...
const Page = () => {
    ...
    const onChange = (e) => {
        ...
        setTodo({
            ...todo, id: crypto.randomUUID(), [name]: [value]
        })
    }
    return (
        <div>
            {count}
            <hr />
            <input type="text" value={value} onChange={(e) => setValue(e.target.value)} className='border border-red-400' />
            <br />
            <button onClick={() => dispatch(increment())}>increment</button>
            <br />
            <button onClick={() => dispatch(decrement())}>decrement</button>
            <br />
            <button onClick={() => dispatch(incrementByAmount(parseInt(value) || 0))}>incrementbyValue</button>
